    ...
    methods: {
        async getMovies() {
            const data = axios.get(
                'https://api.themoviedb.org/3/movie/now_playing?api_key=f2a880af953797f1e72732e30f41db10&language=en-US&page=1'
            )
        ...
        async searchMovies() {
            const data = axios.get(
                `https://api.themoviedb.org/3/search/movie?api_key=f2a880af953797f1e72732e30f41db10&language=en-US&page=1&query=${this.searchInput}`
            )
            const result = await data;
            result.data.results.forEach(movie => {
